...
/**
 * Created by CMDDJ on 4/15/2018.
 */

public class FilterAdapter extends RecyclerView.Adapter<FilterAdapter.ViewHolder> {

//    private List<Store> listOutlet;
//
//    public StoreAdapter(List<Store> listOutlet, Context context) {
...
//    }
    ...
    public FilterAdapter(ArrayList<Store> dataStore, Activity activity) {
...
//    public StoreAdapter(ArrayList<Store> dataStore) {
//        this.dataStore = dataStore;
//    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        //StoreBody storeBody = listOutlet.get(position);
       // Store store = listOutlet.get(position);
        ...
        String jarak = String.format("%.02f", Float.valueOf(dataStore.get(position).getDistanceKilo()))
                .replace(",",".")
                .replace(",00","")
                .replace(".00","")
                .trim();
        ...
        try{
            holder.linkPhoto = dataStore.get(position).getPhoto().get(0).getFile().toString();
        }catch (Exception e){e.printStackTrace();}

        if(holder.linkPhoto != null){
           // Toast.makeText(context, "LINK HEADER : "+holder.linkPhoto, Toast.LENGTH_SHORT).show();
        ...
        if(dataStore.get(position).getSchedule().size() != 0){
            ...
            holder.linearLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                  //  homeFragment.showSpotLoading();

                    Intent intent = new Intent(context, ShowDetailStore.class);
                    Bundle b = new Bundle();
                    b.putString("idStore", holder.strIdStore);
                    b.putString("idUser", holder.strIdUser);
                    intent.putExtras(b);
                    context.startActivity(intent);
                   // Toast.makeText(context, "Salon "+dataStore.get(position).getNameStore().toString(), Toast.LENGTH_SHORT).show();
                    //homeFragment.dismissSpotLoading();
                }
            });
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder{
        ...
        ImageView imgOutlet;
        String strIdStore, strIdUser, linkPhoto;
