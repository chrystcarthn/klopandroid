...
public class MngPhotoAdapter extends BaseAdapter {

    Context context;
    private ArrayList<Photo> dataPhoto;

    public MngPhotoAdapter(ArrayList<Photo> dataPhoto, Context context) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        PhotoAdapter.ViewHolder mHolder;
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_photo, parent, false);
            mHolder = new PhotoAdapter.ViewHolder(convertView);
            convertView.setTag(mHolder);
        } else {
            mHolder = (PhotoAdapter.ViewHolder) convertView.getTag();
        }
